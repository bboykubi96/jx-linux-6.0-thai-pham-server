...
greatseed_configtab = {
	...
	{959,2,42,"\\settings\\maps\\great_night\\chienlongdong_trung.txt","ChiÕn Long §éng"},	-- ³¤°×É½±±¢
	{959,3,30,"\\settings\\maps\\great_night\\chienlongdong_cao.txt","ChiÕn Long §éng"},		-- ³¤°×É½ÄÏ
	{959,4,30,"\\settings\\maps\\great_night\\chienlongdong_hk.txt","ChiÕn Long §éng"},		-- ³¤°×É½ÄÏ
...
function TaskShedule()
	...
	TaskCountLimit(0)
	OutputMsg("=====> HOAT DONG QUA HUY HOANG BAT DAU LUC 21:00");
end

function TaskContent()
	--ÔÚ19:30µ½20:01·ÖÖÐ¼ä²ÅÄÜ´¥·¢
	local START_TIME = 2100;
	local END_TIME = 2130;
	
	if righttime_add() ~= 1 then--Èç¹ûÊ±¼ä²»¶Ô
		gb_SetTask("HOAT DONG HOA DANG", 1, 0)
		gb_SetTask("QUA HUY HOANG", 21, 30);	--12ºÅ±äÁ¿ÓÃÀ´´æ´¢NPCµÄÐòºÅ
		return
	end;
...
function righttime_add()
	...
	if (nTime >= 2100 and nTime < 2130)  then
		return 1;
	end;
	return 0;
end
