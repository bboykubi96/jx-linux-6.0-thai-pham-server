...
--	{336,3,100,"\\settings\\maps\\great_night\\·çÁê¶É.txt","Phong L¨ng §é"},			--·çÁê¶É
	...
	{355,4,6,"\\settings\\maps\\great_night\\chientruongvannien.txt","§Êu Tr­êng Sinh Tö"},		-- ³qua ®¹i hoang kim
	{355,3,9,"\\settings\\maps\\great_night\\chientruonghoangkim.txt","§Êu Tr­êng Sinh Tö"},		-- ³qua hoang kim
...
--	{227,2,40,"\\settings\\maps\\great_night\\É³Ä®É½¶´3.txt","Sa m¹c S¬n §éng 3"},		-- ³¤°×É½ÄÏ
	...
	--{200,1,25,"\\settings\\maps\\great_night\\¹ÅÑô¶´ÃÔ¹¬.txt","C?D­¬ng §éng"},	-- ³¤°×É½ÄÏ
...
function TaskContent()
	...
	local START_TIME = giobatdau;
	local END_TIME = gioketthuc;
	
	if righttime_add() ~= 1 then--Èç¹ûÊ±¼ä²»¶Ô
		gb_SetTask("HOAT DONG HOA DANG", 1, 0)
		gb_SetTask("QUA HUY HOANG", 20, 10);	--12ºÅ±äÁ¿ÓÃÀ´´æ´¢NPCµÄÐòºÅ
		return
	end;
	--AddGlobalCountNews("Tr­íc m¾t l?thêi gian m­a thuËn gi?hßa, ®Ó ®¸p t?c«ng søc ®ãng gãp c¸c v?®¹i hiÖp, V?l©m minh ch?§éc C?KiÕm ®· t?chøc ho¹t ®éng <color=yellow>'§ªm huy hoµng'<color>. Thêi gian b¾t ®Çu ho¹t ®éng mçi ngµy l?20h20.",1)
	...
	if nNowTime >= giobatdau and nNowTime < gioketthuc then	--	
	...
	----- Tr­íc m¾t l?thêi gian m­a thuËn gi?hßa, ®Ó ®¸p t?c«ng søc ®ãng gãp c¸c v?®¹i hiÖp, V?l©m minh ch?§éc C?KiÕm ®· t?chøc ho¹t ®éng <color=yellow>'§ªm huy hoµng'<color>. Thêi gian b¾t ®Çu ho¹t ®éng mçi ngµy l?20h15. <enter><color=yellow>20h15 tr­a nay s?diÔn ra ho¹t ®éng 'Huy chi ch­¬ng'
	--
	-----
	for i = 1, nMapCount do
		...
		if (mod(nBatch,2) == 1) then
			szMsg = format("XuÊt hiÖn, <color=yellow>Administrator<color> ®· gieo <color=green><%s><color> t¹i %s, 5 phót sau c¸c §¹i HiÖp c?th?thu ho¹ch!!",
			szMsg,
			greatseed_configtab[i][5]);
			GlobalExecute(format("dw AddLocalNews([[%s]])",szMsg));
			GlobalExecute(format("dw Msg2SubWorld([[%s]])",szMsg));
		elseif (mod(nBatch,2) == 0) then
			szMsg = format("GÆp thêi tiÕt thuËn lîi <color=pink><%s><color> do <color=yellow>Administrator<color> gieo h¹t ®· kÕt qu¶t¹i <color=green>%s<color>, C¸c §¹i HiÖp h·y nhanh chãng thu ho¹ch!!",
			szMsg,
			greatseed_configtab[i][5]);
			GlobalExecute(format("dw AddLocalNews([[%s]])",szMsg));
			GlobalExecute(format("dw Msg2SubWorld([[%s]])",szMsg));
...
function righttime_add()
	...
	if (nTime >= giobatdau and nTime < gioketthuc)  then
		return 1;
	end;
	return 0;
end
