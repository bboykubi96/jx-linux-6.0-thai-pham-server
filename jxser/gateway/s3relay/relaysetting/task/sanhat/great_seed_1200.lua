...
--{37, 1, 30, "\\settings\\maps\\great_night\\bienkinhquaso.txt","BiÖn Kinh"},
--{37, 2, 30, "\\settings\\maps\\great_night\\bienkinhquatrung.txt","BiÖn Kinh"},
{37, 3, 70, "\\settings\\maps\\great_night\\bienkinhquacao.txt","BiÖn Kinh"},
--{27, 4, 3, "\\settings\\maps\\great_night\\chienlongdong.txt","ChiÕn long ®éng"},
...
TaskInterval(5)
TaskCountLimit(0)
OutputMsg( "=======> Qua Huy Hoang 12h15 <=======");
end

function TaskContent()
local START_TIME = 1230;
local END_TIME = 1255;
if righttime_add() ~= 1 then-- nÕu nhu thêi gian sai
gb_SetTask( "Hoa ®¨ng ho¹t ®éng", 1, 0)
gb_SetTask( "Qu¶ Huy Hoµng", 12, 30);	--19 hµo l­îng biÕn ®æi dïng ®Ó tån tr?NPC t?hµo
return
end;
OutputMsg( "=======> Qua Huy Hoang 12:3 Xuat Hien<=======");
local nNowTime = tonumber(date( "%H%M "))
if nNowTime >= 1230 and nNowTime < 1235 then	--
gb_SetTask( "Qu¶ Huy Hoµng", 12, 30);	--19 hµo l­îng biÕn ®æi dïng ®Ó tån tr?NPC t?hµo
end
...
if (nTime >= 1230 and nTime < 1300 ) then
return 1;
end;
return 0;
end
