...
--	{336,3,100,"\\settings\\maps\\great_night\\·çÁê¶É.txt","Phong L¨ng §é"},			--·çÁê¶É
	...
	{959,4,2,"\\settings\\maps\\great_night\\chienlongdongcao.txt","ChiÕn Long §éng"},		-- ³qua ®¹i hoang kim
	-- {959,3,11,"\\settings\\maps\\great_night\\chienlongdongtrung.txt","ChiÕn Long §éng"},		-- ³qua hoang kim
...
function TaskShedule()
	...
	TaskCountLimit(0)
	OutputMsg("QUA HUY HOANG on start at 20h30 PM");
end
...
function TaskContent()
	...
	if nNowTime >= 2000 and nNowTime < 2030 then	--	
	...
	for i = 1, nMapCount do
		...
		if (mod(nBatch,2) == 1) then
			szMsg = format("XuÊt hiÖn, <color=green>%s<color> t¹i %s, 5 phót sau c¸c §¹i HiÖp cã thÓ thu ho¹ch!!",
			szMsg,
			greatseed_configtab[i][5]);
			GlobalExecute(format("dw AddLocalNews([[%s]])",szMsg));
			GlobalExecute(format("dw Msg2SubWorld([[%s]])",szMsg));
		elseif (mod(nBatch,2) == 0) then
			szMsg = format("GÆp thêi tiÕt thuËn lîi <color=yellow><%s><color>®· kÕt qu¶ t¹i <color=green>%s<color>, C¸c §¹i HiÖp h·y nhanh chãng thu ho¹ch!!",
			szMsg,
			greatseed_configtab[i][5]);
			GlobalExecute(format("dw AddLocalNews([[%s]])",szMsg));
			GlobalExecute(format("dw Msg2SubWorld([[%s]])",szMsg));
