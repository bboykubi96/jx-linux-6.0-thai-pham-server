...
function main()
	...
	if openSkill ~= 1 then
		Say("Ng­¬i ch­a më réng kü n¨ng 150 cÊp 21") 
		return 1
	end
	...
	for i = 1, getn(tb150Skill) do
		...
		if (HaveMagic(nSkillId) == 20) then		-- ±ØÐëÃ»ÓÐ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			if nSkillId == 1110 then
				AddMagic(nSkillId, 0)
			else
				AddMagic(nSkillId, 21)
			end
			if nFlag == 1 then
	...
	if szMsg == "" then
		...
		szMsg = "Ng­¬i ®· nghiªn cøu l·nh héi ®­îc vâ häc, kh«ng thÓ sö dông thªm hoÆc §¼ng cÊp kû n¨ng cÊp 150 cña ng­¬i thÊp h¬n 20,nªn kh«ng thÓ häc yÕu quyÕt nµy."
		Msg2Player(szMsg)
		return 1
	end
end