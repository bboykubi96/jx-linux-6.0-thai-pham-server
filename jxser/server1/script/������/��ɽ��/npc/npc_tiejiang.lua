-- script viet hoa By http://tranhba.com  t©y s¬n tù chøc n¨ng thî rÌn 
-- script viet hoa By http://tranhba.com  By: li_xin(2005-01-27) 
-- script viet hoa By http://tranhba.com update:zhaowenyi(2005-02-20) gia t¨ng mµu tİm cïng hoµng kim trang bŞ chÕ t¹o 

Include("\\script\\global\\×ÏÉ«¼°»Æ½ğ×°±¸ÖıÔì.lua")
function main(sel) 
-- script viet hoa By http://tranhba.com Say("Muèn ë t©y s¬n tù , kh«ng cã ta lîi h¹i binh khİ sî r»ng muèn ¨n thua thiÖt a . ng­¬i muèn mua c¸i g× ? cßn muèn muèn dån t¹o huyÒn tinh trang bŞ hoÆc hoµng kim trang bŞ sÏ tíi t×m ta , gi¸ c¶ nhÊt ®Şnh hîp lı !", 3,"Giao dŞch /yes","ChÕ t¹o /onFoundry","Ch¼ng qua lµ ®i ngang qua /no") 
Say("Muèn ë t©y s¬n tù , kh«ng cã ta lîi h¹i binh khİ sî r»ng muèn ¨n thua thiÖt a . ng­¬i muèn mua c¸i g× ? cßn muèn muèn dån t¹o huyÒn tinh trang bŞ hoÆc hoµng kim trang bŞ sÏ tíi t×m ta , gi¸ c¶ nhÊt ®Şnh hîp lı !", 
2,"Giao dŞch /yes", 
" ch¼ng qua lµ ®i ngang qua /no") 
end; 

function yes() 
Sale(13); 
end; 

function no() 
end; 
