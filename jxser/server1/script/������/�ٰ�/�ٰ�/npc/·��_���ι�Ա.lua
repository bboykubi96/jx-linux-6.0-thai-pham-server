...
function main()
	...
	if (LastFactionNo < 0) then					-- Ã»ÓĞÃÅÅÉµÄ²»ÔÊĞí½øĞĞĞŞ¸´Á÷³Ì£¨°üÀ¨°×Ãû£©
		if (GetFaction() ~= "") then
			...
			Talk(1,"","Ta võa míi tr×nh tÊu víi Hoµng th­îng c¸o l·o vui qóa Ta dù ®Şnh sÏ x©y mét khu hoa viªn ë L©m An ®Ó tËn h­ëng nh÷ng ngµy thanh nhµn cuèi ®êi. ")
		end
		return
	end
	...
	elseif (GetTask(i) == 80*256) and (GetFaction() == faction_name[LastFactionNo+1]) and (GetSeries() == k) and (GetTask(j) == 245) then
		...
		Say("Chµng trai trÎ ®õng v× mét chuyÖn nhá nh­ vËy mµ buån rÇu. B« cùc th¸i lai. ChuyÖn g× råi còng cã thÓ gi¶i quyÕt ®­îc! C?lªn! C?lªn! Kh?kh? ",0)
		SetTaskTemp(20,0)		-- Çå¿ÕÁÙÊ±±äÁ¿£¬ÒÔ±¸ÏÂ´Î
	else
Talk(1,""," §¶m nhiÖm quan viªn  Bæn quan míi võa tõ chøc , ngu dèt Hoµng th­îng ©n chuÈn , c¸o l·o vÒ quª , ta chuÈn bŞ ë tr­íc khi an ngo¹i « x©y mét khu nhµ v­ên , thËt tèt h­ëng mÊy n¨m thanh phóc . ") 
	end
end;
