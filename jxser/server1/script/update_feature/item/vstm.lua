...
-------------------------------------------------------------------------------
TSK_TIMEDATE_TM = 2764; TSK_TIMELIMIT_TM = 7*24*60*60
-------------------------------------------------------------------------------
function main()
local nNowTime = GetCurServerTime()
local nTimeDate = GetTask(TSK_TIMEDATE_TM)
	if (nTimeDate + TSK_TIMELIMIT_TM <= nNowTime) then
		SetTask(TSK_TIMEDATE_TM,nNowTime) PlayerFunLib:AddSkillState(1616,20,3,10886400,1) ; RemoveSkillState(1614,20,3,10886400,1); RemoveSkillState(1615,20,3,10886400,1) 
		Msg2Player("Chóc Mõng Ng­¬i §· NhËn §­îc HiÖu øng Vßng S¸ng TriÖu M·nh Cã HiÖu Lùc Trong 7 Ngµy")
            logplayer("dulieu/vongsang/kichhoatvongsangtrieumanh_.txt",format(" Thêi gian : %s  - Tµi kho¶n [ %s] - Nh©n vËt : [%s ] - Kich Hoat Vong Sang trieu manh\n! ",GetLocalDate("%m/%d/%Y_%H:%M:%S"),GetAccount(),GetName()))
