...
function tbQuyNguyenXichLanMax:Compose(nComposeCount)
	...
	if tbQuyNguyenXichLanMaxList[nEquType].nWidth ~= 0 and tbQuyNguyenXichLanMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbQuyNguyenXichLanMaxList[nEquType].nWidth, tbQuyNguyenXichLanMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbQuyNguyenXichLanMaxList[nEquType].nWidth, tbQuyNguyenXichLanMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngò Linh Quy Nguyªn Phï", tbProp = {6,1,nID_IDX_QuyNguyenPhu,1,0,0}, nCount = tbQuyNguyenXichLanMaxList[nEquType].nStone}
...
function QuyNguyenXichLanMax()
	...
	local tbMate = {
		tbMaterial = {
			{szName = "Trang bÞ XÝch L©n", tbProp = {0, {IDX_XichLanMax_nStart, IDX_XichLanMax_nEnd}}, nQuality = 1},
			{szName = "Ngò Linh Quy Nguyªn Phï", tbProp = {6,1,IDX_QuyNguyenPhu,1,0,0},nCount = 1},
		},
		tbProduct = {szName="B¶o R­¬ng XÝch L©n", tbProp={0, {IDX_BaoRuongXichLanMax_Start, IDX_BaoRuongXichLanMax_nEnd}}},
	}
