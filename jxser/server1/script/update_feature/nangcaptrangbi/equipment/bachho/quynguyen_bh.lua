...
------------------------------------------------------------------------------------------
tbQuyNguyenBachHo = tbActivityCompose:new()
tbQuyNguyenBachHo.bAccessBindItem = 0

tbQuyNguyenBachHoList = {
...
function tbQuyNguyenBachHo:ComposeGiveUI()
...
function tbQuyNguyenBachHo:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbQuyNguyenBachHoList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbQuyNguyenBachHoList[nEquType].nWidth ~= 0 and tbQuyNguyenBachHoList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbQuyNguyenBachHoList[nEquType].nWidth, tbQuyNguyenBachHoList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbQuyNguyenBachHoList[nEquType].nWidth, tbQuyNguyenBachHoList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngò Linh Quy Nguyªn Phï", tbProp = {6,1,IDX_QuyNguyenPhu,1,0,0}, nCount = tbQuyNguyenBachHoList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbQuyNguyenBachHoList[nEquType].nStone then
	...
	local tbProduct = BaoRuongBachHo[nEquType][nIdx]
...
function QuyNguyenBachHo()
	...
	local tbMate = {
		...
		tbProduct = {szName="B¶o R­¬ng B¹ch Hæ", tbProp={0, {IDX_BaoRuongBachHo_Start, IDX_BaoRuongBachHo_nEnd}}},
	}
	local p = tbQuyNguyenBachHo:new(tbMate, "tbQuyNguyenBachHo", INVENTORY_ROOM.room_giveitem)
