...
function tbQuyNguyenDangLongMax:Compose(nComposeCount)
	...
	if tbQuyNguyenDangLongMaxList[nEquType].nWidth ~= 0 and tbQuyNguyenDangLongMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbQuyNguyenDangLongMaxList[nEquType].nWidth, tbQuyNguyenDangLongMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbQuyNguyenDangLongMaxList[nEquType].nWidth, tbQuyNguyenDangLongMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngò Linh Quy Nguyªn Phï", tbProp = {6,1,IDX_QuyNguyenPhu,1,0,0}, nCount = tbQuyNguyenDangLongMaxList[nEquType].nStone}
...
function QuyNguyenDangLongMax()
	...
	local tbMate = {
		tbMaterial = {
			{szName = "Trang bÞ §»ng Long", tbProp = {0, {IDX_DangLongMax_nStart, IDX_DangLongMax_nEnd}}, nQuality = 1},
