...
--------------------------------------------------------------------
tbTrungLuyenAnhHao = tbActivityCompose:new()

tbTrungLuyenAnhHaoList = {
...
function tbTrungLuyenAnhHao:ComposeGiveUI()
...
function tbTrungLuyenAnhHao:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbTrungLuyenAnhHaoList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenAnhHaoList[nEquType].nWidth ~= 0 and tbTrungLuyenAnhHaoList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenAnhHaoList[nEquType].nWidth, tbTrungLuyenAnhHaoList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenAnhHaoList[nEquType].nWidth, tbTrungLuyenAnhHaoList[nEquType].nHeight))
		return 0
	end	

local tbItem = {szName = "Xu ", tbProp = {4,417,1,1,0,0},  nCount = tbTrungLuyenAnhHaoList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenAnhHaoList[nEquType].nStone then
	...
	local tbProduct = tbEquip_AnhHao[nEquType][nIdx]
...
function TrungLuyenAnhHao()
	...
	local tbMate = {
		...
		tbProduct = {szName="Trang bÞ Anh Hµo", tbProp={0, {IDX_AnhHao_nStart, IDX_AnhHao_nEnd}}},
	}
	local p = tbTrungLuyenAnhHao:new(tbMate, "tbTrungLuyenAnhHao", INVENTORY_ROOM.room_giveitem)
