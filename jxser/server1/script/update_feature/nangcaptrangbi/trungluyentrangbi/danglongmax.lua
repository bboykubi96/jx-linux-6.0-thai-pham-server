...
function tbTrungLuyenDangLongMax:Compose(nComposeCount)
	...
	if tbTrungLuyenDangLongMaxList[nEquType].nWidth ~= 0 and tbTrungLuyenDangLongMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenDangLongMaxList[nEquType].nWidth, tbTrungLuyenDangLongMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenDangLongMaxList[nEquType].nWidth, tbTrungLuyenDangLongMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "§»ng Long Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc_DangLong,1,0,0}, nCount = tbTrungLuyenDangLongMaxList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenDangLongMaxList[nEquType].nStone then
		local szMsg = self.tbFormula.szFailMsg or "<color=red>§»ng Long Trïng LuyÖn Ngäc mµ ng­¬i ®Æt vµo kh«ng ®ñ. <color>"
		Talk(1,"", szMsg)
		return 0;
	end
...
function TrungLuyenDangLongMax()
	...
	local tbMate = {
		tbMaterial = {
			...
			{szName = "§»ng Long Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc_DangLong,1,0,0}},
		},
		tbProduct = {szName="Trang bÞ §»ng Long", tbProp={0, {IDX_DangLongMax_nStart, IDX_DangLongMax_nEnd}}},
	}
