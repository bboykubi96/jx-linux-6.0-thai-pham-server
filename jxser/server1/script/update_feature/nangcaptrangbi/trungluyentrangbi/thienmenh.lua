...
--------------------------------------------------------------------
tbTrungLuyenThienMenh = tbActivityCompose:new()

tbTrungLuyenThienMenhList = {
...
function tbTrungLuyenThienMenh:ComposeGiveUI()
...
function tbTrungLuyenThienMenh:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbTrungLuyenThienMenhList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenThienMenhList[nEquType].nWidth ~= 0 and tbTrungLuyenThienMenhList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenThienMenhList[nEquType].nWidth, tbTrungLuyenThienMenhList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenThienMenhList[nEquType].nWidth, tbTrungLuyenThienMenhList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}, nCount = tbTrungLuyenThienMenhList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenThienMenhList[nEquType].nStone then
	...
	local tbProduct = tbEquip_ThienMenh[nEquType][nIdx]
...
function TrungLuyenThienMenh()
	...
	local tbMate = {
		...
		tbProduct = {szName="Trang bÞ Thiªn MÖnh", tbProp={0, {IDX_ThienMenh_nStart, IDX_ThienMenh_nEnd}}},
	}
	local p = tbTrungLuyenThienMenh:new(tbMate, "tbTrungLuyenThienMenh", INVENTORY_ROOM.room_giveitem)
