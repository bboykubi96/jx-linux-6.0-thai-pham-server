...
function tbTrungLuyenBachHoMax:Compose(nComposeCount)
	...
	if tbTrungLuyenBachHoMaxList[nEquType].nWidth ~= 0 and tbTrungLuyenBachHoMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenBachHoMaxList[nEquType].nWidth, tbTrungLuyenBachHoMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenBachHoMaxList[nEquType].nWidth, tbTrungLuyenBachHoMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "B¹ch Hæ Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc_BachHo,1,0,0}, nCount = tbTrungLuyenBachHoMaxList[nEquType].nStone}
...
function TrungLuyenBachHoMax()
	...
	local tbMate = {
		tbMaterial = {
			...
			{szName = "Ngäc Trïng LuyÖn B¹ch Hæ", tbProp = {6,1,IDX_TrungLuyenNgoc_BachHo,1,0,0}},
		},
		tbProduct = {szName="Trang bÞ B¹ch Hæ", tbProp={0, {IDX_BachHoMax_nStart, IDX_BachHoMax_nEnd}}},
	}
