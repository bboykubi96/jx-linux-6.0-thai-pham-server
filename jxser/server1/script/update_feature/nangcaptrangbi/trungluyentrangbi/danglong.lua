...
--------------------------------------------------------------------
tbTrungLuyenDangLong = tbActivityCompose:new()

tbTrungLuyenDangLongList = {
...
function tbTrungLuyenDangLong:ComposeGiveUI()
	local szTitle = format("%s","Trïng LuyÖn §»ng Long")
	...
	strDesc = strDesc..format("%-22s %s\n","§»ng Long H¹ng Liªn","1")
	strDesc = strDesc..format("%-22s %s\n","§»ng Long Giíi ChØ","2")
	strDesc = strDesc..format("%-22s %s","§»ng Long Vò KhÝ","4")
	
	g_GiveItemUI(szTitle, strDesc, {self.GiveUIOk, {self}}, nil, self.bAccessBindItem)
end

function tbTrungLuyenDangLong:Compose(nComposeCount)
	...
	if tbGoldEquip == nil then
		Say("Ng­¬i Ch­a §Æt §óng Trang BÞ §»ng Long")
		return 0	
	end
		
	local nFreeItemCellLimit = tbTrungLuyenDangLongList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenDangLongList[nEquType].nWidth ~= 0 and tbTrungLuyenDangLongList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenDangLongList[nEquType].nWidth, tbTrungLuyenDangLongList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenDangLongList[nEquType].nWidth, tbTrungLuyenDangLongList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}, nCount = tbTrungLuyenDangLongList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenDangLongList[nEquType].nStone then
	...
	local tbProduct = tbEquip_DangLong[nEquType][nIdx]
...
function TrungLuyenDangLong()
	local strDesc = "<color=green>Muèn Trïng LuyÖn <color=yellow>Trang BÞ §»ng Long<color> Yªu CÇu:<color>\n"
	strDesc = strDesc..format("\t\t\t%-30s %s\n", "Trang BÞ §»ng Long", "Ngäc Trïng LuyÖn")
	...
	local tbMate = {
		tbMaterial = {
			...
			{szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}},
		},
		tbProduct = {szName="Trang bÞ §»ng Long", tbProp={0, {IDX_DangLong_nStart, IDX_DangLong_nEnd}}},
	}
	local p = tbTrungLuyenDangLong:new(tbMate, "tbTrungLuyenDangLong", INVENTORY_ROOM.room_giveitem)
