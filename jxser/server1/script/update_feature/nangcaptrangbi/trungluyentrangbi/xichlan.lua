...
function tbTrungLuyenXichLan:Compose(nComposeCount)
	...
	if tbTrungLuyenXichLanList[nEquType].nWidth ~= 0 and tbTrungLuyenXichLanList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenXichLanList[nEquType].nWidth, tbTrungLuyenXichLanList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenXichLanList[nEquType].nWidth, tbTrungLuyenXichLanList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}, nCount = tbTrungLuyenXichLanList[nEquType].nStone}
...
function TrungLuyenXichLan()
	...
	local tbMate = {
		...
		tbProduct = {szName="Trang bÞ XÝch L©n", tbProp={0, {IDX_XichLan_nStart, IDX_XichLan_nEnd}}},
	}
