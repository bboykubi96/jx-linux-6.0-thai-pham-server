...
--------------------------------------------------------------------
tbTrungLuyenNguyetKhuyetMax = tbActivityCompose:new()

tbTrungLuyenNguyetKhuyetMaxList = {
...
function tbTrungLuyenNguyetKhuyetMax:ComposeGiveUI()
...
function tbTrungLuyenNguyetKhuyetMax:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbTrungLuyenNguyetKhuyetMaxList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenNguyetKhuyetMaxList[nEquType].nWidth ~= 0 and tbTrungLuyenNguyetKhuyetMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenNguyetKhuyetMaxList[nEquType].nWidth, tbTrungLuyenNguyetKhuyetMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("ß” b∂o Æ∂m tµi s∂n cÒa Æπi hi÷p, xin h∑y Æ” trËng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenNguyetKhuyetMaxList[nEquType].nWidth, tbTrungLuyenNguyetKhuyetMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Nguy÷t Khuy’t Ng‰c TrÔng Luy÷n", tbProp = {6,1,IDX_TrungLuyenNgoc_NguyetKhuyet,1,0,0}, nCount = tbTrungLuyenNguyetKhuyetMaxList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenNguyetKhuyetMaxList[nEquType].nStone then
		local szMsg = self.tbFormula.szFailMsg or "<color=red>Nguy÷t Khuy’t TrÔng Luy÷n Ng‰c mµ ng≠¨i Æ∆t vµo kh´ng ÆÒ. <color>"
		Talk(1,"", szMsg)
		return 0;
	end
	
	local nIdx = floor((nEqIndex - IDX_NguyetKhuyet_nStart)/10) + 1
	local tbProduct = tbEquip_NguyetKhuyetMax[nEquType][nIdx]
...
function TrungLuyenNguyetKhuyetMax()
	...
	local tbMate = {
		tbMaterial = {
			...
			{szName = "Nguy÷t Khuy’t Ng‰c TrÔng Luy÷n", tbProp = {6,1,IDX_TrungLuyenNgoc_NguyetKhuyet,1,0,0}},
		},
		tbProduct = {szName="Trang bﬁ Nguy÷t Khuy’t", tbProp={0, {IDX_NguyetKhuyetMax_nStart, IDX_NguyetKhuyetMax_nEnd}}},
	}
	local p = tbTrungLuyenNguyetKhuyetMax:new(tbMate, "tbTrungLuyenNguyetKhuyetMax", INVENTORY_ROOM.room_giveitem)
