...
--------------------------------------------------------------------
tbTrungLuyenBachHo = tbActivityCompose:new()

tbTrungLuyenBachHoList = {
...
function tbTrungLuyenBachHo:ComposeGiveUI()
	local szTitle = format("%s","Trïng LuyÖn B¹ch Hæ")
	...
	strDesc = strDesc..format("%-22s %s\n","B¹ch Hæ H¹ng Liªn","1")
	strDesc = strDesc..format("%-22s %s\n","B¹ch Hæ Giíi ChØ","2")
	strDesc = strDesc..format("%-22s %s","B¹ch Hæ Vò KhÝ","4")
	
	g_GiveItemUI(szTitle, strDesc, {self.GiveUIOk, {self}}, nil, self.bAccessBindItem)
end

function tbTrungLuyenBachHo:Compose(nComposeCount)
	...
	if tbGoldEquip == nil then
		Say("Ng­¬i Ch­a §Æt §óng Trang BÞ B¹ch Hæ")
		return 0	
	end
		
	local nFreeItemCellLimit = tbTrungLuyenBachHoList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenBachHoList[nEquType].nWidth ~= 0 and tbTrungLuyenBachHoList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenBachHoList[nEquType].nWidth, tbTrungLuyenBachHoList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenBachHoList[nEquType].nWidth, tbTrungLuyenBachHoList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}, nCount = tbTrungLuyenBachHoList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenBachHoList[nEquType].nStone then
	...
	local tbProduct = tbEquip_BachHo[nEquType][nIdx]
...
function TrungLuyenBachHo()
	local strDesc = "<color=green>Muèn Trïng LuyÖn <color=yellow>Trang BÞ B¹ch Hæ<color> Yªu CÇu:<color>\n"
	strDesc = strDesc..format("\t\t\t%-30s %s\n", "Trang BÞ B¹ch Hæ", "Ngäc Trïng LuyÖn")
	...
	local tbMate = {
		tbMaterial = {
			{szName = "Trang bÞ B¹ch Hæ", tbProp = {0, {IDX_BachHo_nStart, IDX_BachHo_nEnd}}, nQuality = 1},
			{szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}},
		},
		tbProduct = {szName="Trang bÞ B¹ch Hæ", tbProp={0, {IDX_BachHo_nStart, IDX_BachHo_nEnd}}},
	}
	local p = tbTrungLuyenBachHo:new(tbMate, "tbTrungLuyenBachHo", INVENTORY_ROOM.room_giveitem)
