...
--------------------------------------------------------------------
tbTrungLuyenDieuDuongMax = tbActivityCompose:new()

tbTrungLuyenDieuDuongMaxList = {
...
function tbTrungLuyenDieuDuongMax:ComposeGiveUI()
...
function tbTrungLuyenDieuDuongMax:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbTrungLuyenDieuDuongMaxList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenDieuDuongMaxList[nEquType].nWidth ~= 0 and tbTrungLuyenDieuDuongMaxList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenDieuDuongMaxList[nEquType].nWidth, tbTrungLuyenDieuDuongMaxList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("ß” b∂o Æ∂m tµi s∂n cÒa Æπi hi÷p, xin h∑y Æ” trËng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenDieuDuongMaxList[nEquType].nWidth, tbTrungLuyenDieuDuongMaxList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Di÷u D≠¨ng Ng‰c TrÔng Luy÷n", tbProp = {6,1,IDX_TrungLuyenNgoc_DieuDuong,1,0,0}, nCount = tbTrungLuyenDieuDuongMaxList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenDieuDuongMaxList[nEquType].nStone then
		local szMsg = self.tbFormula.szFailMsg or "<color=red>Di÷u D≠¨ng TrÔng Luy÷n Ng‰c mµ ng≠¨i Æ∆t vµo kh´ng ÆÒ. <color>"
		Talk(1,"", szMsg)
		return 0;
	end
	
	local nIdx = floor((nEqIndex - IDX_DieuDuong_nStart)/10) + 1
	local tbProduct = tbEquip_DieuDuongMax[nEquType][nIdx]
...
function TrungLuyenDieuDuongMax()
	...
	local tbMate = {
		tbMaterial = {
			...
			{szName = "Di÷u D≠¨ng Ng‰c TrÔng Luy÷n", tbProp = {6,1,IDX_TrungLuyenNgoc_DieuDuong,1,0,0}},
		},
		tbProduct = {szName="Trang bﬁ Di÷u D≠¨ng", tbProp={0, {IDX_DieuDuongMax_nStart, IDX_DieuDuongMax_nEnd}}},
	}
	local p = tbTrungLuyenDieuDuongMax:new(tbMate, "tbTrungLuyenDieuDuongMax", INVENTORY_ROOM.room_giveitem)
