...
--------------------------------------------------------------------
tbTrungLuyenMinhPhuong = tbActivityCompose:new()

tbTrungLuyenMinhPhuongList = {
...
function tbTrungLuyenMinhPhuong:ComposeGiveUI()
...
function tbTrungLuyenMinhPhuong:Compose(nComposeCount)
	...
	local nFreeItemCellLimit = tbTrungLuyenMinhPhuongList[nEquType].nFreeItemCellLimit or 1
	nFreeItemCellLimit = ceil(nFreeItemCellLimit * nComposeCount)
	if tbTrungLuyenMinhPhuongList[nEquType].nWidth ~= 0 and tbTrungLuyenMinhPhuongList[nEquType].nHeight ~= 0 and CountFreeRoomByWH(tbTrungLuyenMinhPhuongList[nEquType].nWidth, tbTrungLuyenMinhPhuongList[nEquType].nHeight, nFreeItemCellLimit) < nFreeItemCellLimit then
		Say(format("§Ó b¶o ®¶m tµi s¶n cña ®¹i hiÖp, xin h·y ®Ó trèng %d %dx%d hµnh trang", nFreeItemCellLimit, tbTrungLuyenMinhPhuongList[nEquType].nWidth, tbTrungLuyenMinhPhuongList[nEquType].nHeight))
		return 0
	end	

	local tbItem = {szName = "Ngäc Trïng LuyÖn", tbProp = {6,1,IDX_TrungLuyenNgoc,1,0,0}, nCount = tbTrungLuyenMinhPhuongList[nEquType].nStone}
	local nCount = self:CalcItemCount(tbRoomItems, tbItem)
	if nCount < tbTrungLuyenMinhPhuongList[nEquType].nStone then
	...
	local tbProduct = tbEquip_MinhPhuong[nEquType][nIdx]
...
function TrungLuyenMinhPhuong()
	...
	local tbMate = {
		...
		tbProduct = {szName="Trang bÞ Minh Ph­îng", tbProp={0, {IDX_MinhPhuong_nStart, IDX_MinhPhuong_nEnd}}},
	}
	local p = tbTrungLuyenMinhPhuong:new(tbMate, "tbTrungLuyenMinhPhuong", INVENTORY_ROOM.room_giveitem)
