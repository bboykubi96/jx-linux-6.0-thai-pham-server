...
NPC_Clickdb = {
	{1556,80,4,959,1613,2953,0,"Trèng Kh¶i Hoµn Bang Héi",0,"\\script\\update_feature\\trongbanghoi\\trongbanghoi.lua","main", 0},
      
       
}
function add_npc_click3()
local nHour = tonumber(GetLocalDate("%H%M"))
		if( nHour >= 2005 and nHour <= 2030) then
...
local _Limit = function(nNpcIdx)
	...
	if(nTongID == 0) then
		Msg2Player("Ch­a gia nhËp <color=yellow>Bang Héi<color>, hoÆc kh«ng ph¶i lµ <color=yellow>Bang Chñ<color> kh«ng thÓ tham gia ®¸nh trèng!");
		return
                      end
	local szTongName, nTongID = GetTongName();
	local figure = TONGM_GetFigure(nTongID, GetName())
	if (figure ~= TONG_MASTER) then
		Msg2Player("<color=wood>ChØ cã <color=yellow>Bang Chñ<color> míi cã ®ñ t­ c¸ch ®¸nh trèng.<color>");
		return
	end;
...
local _GetFruit = function(nNpcIdx, dwNpcId)
	...
	--SetPKFlag(0);
	--ForbidChangePK(0)
	Msg2SubWorld("<color=fire>Bang Héi <color=wood>["..GetTong().."]<color> ®· ®¸nh thµnh c«ng Trèng Bang Héi mang vÒ nh÷ng vËt phÈm quý gi¸ cho bæn bang!")
end
...
function main()
	...
	Say("Ch­a tíi thêi gian ®¸nh trèng h·y mau tËp trung binh lùc ®Ó b¶o vÖ Trèng Kh¶i Hoµn.",0);
	return 1;
end
	...
	Msg2SubWorld("Bang Héi <color=yellow>["..GetTong().."]<color> ®ang tiÕn hµnh ®¸nh Trèng h·y mau ng¨n c¶n!")
end;
