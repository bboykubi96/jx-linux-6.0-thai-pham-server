...
local PK_value = GetPK(); 
local my_money = GetCash(); 
Say("Nha dŞch # ng­¬i hai tay dİnh cã m¸u tanh , nhÊt ®Şnh ph¶i tr¶i qua quan phñ gi¸m ®Şnh , m×nh ë ®¹i lao trong tÜnh t©m hèi c·i l¹i võa chuéc téi . <enter>……<enter> hiÖn ph¸n quyÕt nh­ sau #<enter>## nªn ph¹m <color=red> "..GetName().." <color> b©y giê PK trŞ gi¸ lµ <color=red>"..PK_value.."<color> , kh«ng cã thu nªn ph¹m së mang toµn bé tang kho¶n <color=red>"..my_money.."<color> hai , nhèt vµo ®¹i lao hèi c·i <color=red>"..PK_value.."<color> ngµy . ", 2,"TiÕn vµo ®¹i lao /go_atone","Cßn lµ lÎn trèn ®i /no"); 
end; 
function auto_atone()
do return end-- dong nho tu tu dong by Zhu
if GetPK() <=3 then
return
end
print("Nhot tu "..GetName())
Msg2SubWorld("¤ng th©n "..GetName().."t¸c oai t¸c qu¸i ®i lang thang ®· bŞ quan sai b¾t v« tï. Lêi c¶nh tØnh cho nh÷ng ai cßn ch­a tØnh ngé. Tõ ®©y giang hå ®· mÊt mét kÎ manh ®éng. ThËt ®¸ng tiÕc")
go_atone() 
return 1
end
function go_atone() 
...
if login_add_new then 
login_add_new(auto_atone,1)
print("Tu dong bo tu khi dang nhap") 
end