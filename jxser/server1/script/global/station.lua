...
Include("\\script\\task\\tollgate\\messenger\\wagoner.lua");  --ÌØÊâĞÅÊ¹µØÍ¼
Include("\\script\\global\\g7vn\\g7configall.lua")
Include("\\script\\task\\newtask\\map_index.lua"); -- ÓÃÓÚ»ñÈ¡ÈÎÎñÁ´µØÍ¼µÄĞÅÏ¢
Include("\\script\\task\\tollgate\\messenger\\wagoner.lua") --ÓÃÓÚÁ¬½ÓÌØÊâĞÅÊ¹ÈÎÎñµÄ½Å±¾
...
szStationOp = 
	{
		...
		"Nh÷ng thµnh thŞ ®· ®i qua/StationFun",		-- 2
		"Trë l¹i ®Şa ®iÓm cò /TownPortalFun", 	-- 3
		...
		"§i n¬i ®Æc biÖt lµm NhiÖm vô Tİn Sø /messenger_wagoner",      -- 9
		--"È¥ÎŞÃû¹È/GotoAnonymVale"			--10
	};
...
-- ¶ÔÓ¦ÉÏ¸öÊı×éµÄÌõ¼şÅĞ¶Ï
aryCondition = {

	[1] = {checkState = return_1},     -- Â·¾­µÄµØ·½
	[2] = {checkState = return_1},     -- Â·¾­µÄ³ÇÊĞ
	...
	[9] = {checkState = function()                      -- È¥ÌØÊâĞÅÊ¹ÈÎÎñËùÔÚµØµã
				return (GetTask(1204)~= 0 and 1) or 0;
		   end;
		  },
	--[10] = {checkState = return_1},    -- ½øÈëÎŞÃû¹È
...
function go_HSBattle()
	Msg2Player("Ngåi ch¾c nhĞ! Chóng ta ®i ®Õn Hoa s¬n c¶nh kü tr­êng")
...
function WayPointFun()
	...
	--if PK_value == 10 then
	--	Say("Ng­êi hai tay dİnh ®Çy m¸u trŞ sè <color=red>PK 10<color> ®i ®Õn ch©n trêi gãc bÓ th× còng kh«ng thÓ tho¸t khái luËt ph¸p! <enter>Mau mau ®Õn nhµ lao ®Çu thó ®Ó gi¶m bít trŞ PK", 0);
	--	return 1
	--end
...
function CancelGoCityWar()
	Say("ChiÕn tr­êng §ao KiÕm v« t×nh!  Xin nghÜa sÜ h·y quay vÒ ®Ó b¶o toµn tİnh m¹ng", 0);
end;
...
function GoDemoWar()
	Say("C«ng thµnh ®ang s¾p b¾t ®Çu!  Ng­¬i cã muèn tham gia kh«ng?", 3,"Ta tham gia bªn thñ!/GotoDefend", "Ta tham gia bªn c«ng/GotoAttack", "Ch­a chuÈn bŞ tèt, lÇn sau ®i!/OnCancel") 
end;
...
function GotoClearSkillMap()
	-- ÔİÊ±ÆÁ±ÎÈ¥Ï´µãµØÍ¼µÄ¹¦ÄÜ
	-- Say("³µ·ò£ºÃâ·ÑÈ¥Ï´ËèµºµÄ»ú»áÖ»ÓĞÒ»´Î¡£Äã×îºÃ»¹ÊÇÏÈÊìÏ¤Ò»ÏÂÕâ´ÎµÄ¼¼ÄÜµ÷Õû£¬³ÙÒ»µãÔÙÈ¥°É¡£", 1, "ÖªµÀÁË/gotocsm_no")
...
CITY_OPTIONS =
{
	...
	--{"§i Thİ LuyÖn Cèc",				goto_shijiangu},		-- 11
...
-- ³ÇÊĞ³µ·òÍ¨ÓÃ½Å±¾
-- ×¢Òâ£º¹¥³ÇÕ½Î´Ê¹ÓÃ´Ëº¯ÊıµÄ½Å±¾ [script\missions\citywar_city\zhongzhuan_map\chefu.lua]
...
local zDate = tonumber(date("%Y%m%d%H%M"))
	...
	if GetExp()<0 then
--		Say("Kinh nghiÖm bŞ ©m. kh«ng thÓ ®i xa phu.", 0);
	--	return 1
	end
	...
	for i = 1, getn(CITY_OPTIONS) do
		...
		-- µ÷ÓÃ aryCondition ÄÚµÄ checkState ¹ı³Ì£¬¼ì²é´Ë¶Ô»°Ñ¡ÏîÊÇ·ñ·ûºÏÌõ¼ş
...
-- ĞÂÊÖ´å³µ·òÍ¨ÓÃ½Å±¾ (×¢Òâ£ºÄÏÔÀÕò²»ÊÇĞÂÊÖ´å£¬²»µ÷´Ëº¯Êı)
...
local zDate = tonumber(date("%Y%m%d%H%M"))
	...
	--	SetPos(1628, 3100)
		return 1
	end;				-- ¼¼ÄÜ¸üĞÂ¡¢ÃÅÅÉ¼Ó±êÊ¶£¨2004-05-31£©
	if GetExp()<0 then
	--	Say("Kinh nghiÖm bŞ ©m. kh«ng thÓ ®i xa phu.", 0);
	--	return 1
	end
...
return Say("§ang VËn Tiªu Xin §õng GÆp Ta")
	...
	if (GetLevel() >= 5) then
		...
		elseif (GetGlbValue(2) == 1) then -- ¹¥³ÇÑµÁ·Õ½³¡
			...
			if (GetTask(1021) == 4) then -- Èç¹û½ÓÊÜÁËÒ°ÛÅÈÎÎñÖĞÑ°ÕÒµØÍ¼Ö¾µÄÈÎÎñ
		...
		Talk(1,"","Bªn ngoµi rÊt lo¹n l¹c, xem bé d¹ng kh«ng c¶n ®­îc giã cña ng­¬i th× khã mµ b¶o toµn tİnh m¹ng! ")
	end
end;
...
function transtoplace_sure()
	...
	if (GetLastFactionNumber() == -1) then
		Say("Vâ l©m minh chñ cã lÖnh: Gi¶i thi ®Êu 'KiÖt xuÊt' cña vâ l©m m«n ph¸i chØ h¹n chÕ cho 10 ®¹i m«n ph¸i tham gia. Ng­¬i kh«ng cã m«n ph¸i nµo hÕt trong vâ l©m, kh«ng thÓ tïy tiÖn tham gia ®­îc, mau gia nhËp thËp ®¹i m«n ph¸i, råi ®Õn ®¨ng kı sau!", 0)
		return
	else
