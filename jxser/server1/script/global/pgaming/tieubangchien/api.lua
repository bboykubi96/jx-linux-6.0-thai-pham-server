...
function m_TieuBangChien:api_Station(_1)
	...
	elseif _1 and _1 == 2 then
		...
		if b_Id > 0 and b_N ~= "" and b_N ~= nil then
			if self.m.StateWithNotMap_Pos.a[b_N] then
				...
				Msg2Tong(b_Id, GetName().." <color=fire>§· ®¨ng ký Tiªu Bang ChiÕn, bang héi cña b¹n ®­îc chØ ®Þnh ®ång chiÕm lÜnh khu vùc "..c)
			end
		end
	...
	if self:api_CheckForbidItem() then
		return Say("Khu vùc nµy lµ chiÕn sù c«ng b»ng, mäi ®èi thñ vµ ®ång minh ®Òu kh«ng ®­îc phÐp mang theo bÊt cø vËt phÈm hç trî nµo, c¸c h¹ xin kiÓm tra l¹i hµnh trang, nÕu cã c¸c lo¹i bµo hoµn hoÆc vËt phÈm bæ trî xin h·y lÊy ra!", 0)
	end
