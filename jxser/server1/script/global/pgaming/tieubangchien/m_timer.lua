...
function m_TieuBangChien:SortRank(_TimerLoop)
	...
	pg.Msg2AllWorld("<color=0x9BFF9B>TBC <---[ ChÕt nhiÒu nhÊt ]--->")
	for _i = 1, 10 do
		if not(b[_i]) then
			break
		end
		...
		if c < 3 then
			c = c + 1
			pg.Msg2AllWorld("<color=0x9BFF9B>".._i.."."..b[_i].n.." - ChÕt "..b[_i].d.." lÇn")
		end
	end
	...
	c = 0
	pg.Msg2AllWorld("<color=0xa9ffe0>TBC <---[ TOP Liªn Tr¶m ]--->")
	for _i = 1, 10 do
		if not(b[_i]) then
			break
		end
		...
		if c < 3 then
			c = c + 1
			pg.Msg2AllWorld("<color=0xa9ffe0>".._i.."."..b[_i].n.." - Liªn "..b[_i].l.."")
		end
	end
	
	sort(b, function(a,b) return(a.p > b.p) end)
	
	pg.Msg2AllWorld("<color=0x9BFF9B>TBC <---[ TOP S¸t Thñ ]--->")
	for _i = 1, 10 do
		if not(b[_i]) then
			break
		end
		...
		pg.Msg2AllWorld("<color=0x9BFF9B>".._i.."."..b[_i].n.." - PK "..b[_i].p.."")
	end
...
function m_TieuBangChien:AddTiepTe()
	...
	Msg2MSAll(self.m.ID, pg.C(10, "C¸c ®iÓm "..(self.Add.TiepTe[a].n and self.Add.TiepTe[a].n or "TiÕp tÕ").." ®· xuÊt hiÖn, h·y mau chãng tiÕp søc cho b¶n th©n!"))
end
