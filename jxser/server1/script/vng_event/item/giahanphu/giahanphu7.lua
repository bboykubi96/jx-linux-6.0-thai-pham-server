...
function tbVnGiaHanPhuGiveUIConfirm(nCount)
	...
	--	Talk(1, "", "ChØ cã thÓ gia h¹n trang søc cã h¹n sö dông d­íi 2 ngµy.")
	--	return
	--end
	if ConsumeEquiproomItem(1, 6,1,4910,1) ~= 1 then
		Talk(1, "", "Kh«ng t×m thÊy vËt phÈm Gia H¹n Phï, gia h¹n thÊt b¹i.")
		return
	end
	local strItemName = GetItemName(nIDX)
		RemoveItemByIndex(nIDX)
		if nGoldEquipIdx== 3956 then 
	tbAwardTemplet:GiveAwardByList({{szName = strItemName,tbProp={0,6061},nQuality=1,nCount=1,nExpiredTime=(nTimeLeftCount+30)*24*60},}, "test", 1);
	else 
	tbAwardTemplet:GiveAwardByList({{szName = strItemName,tbProp={0,6068},nQuality=1,nCount=1,nExpiredTime=(nTimeLeftCount+30)*24*60},}, "test", 1);
	end
...
dofile("script/vng_event/item/giahanphu/giahanphu7.lua")
	local nCheckMap = DynamicExecuteByPlayer(PlayerIndex, "\\script\\vng_feature\\checkinmap.lua", "PlayerFunLib:VnCheckInCity")
	if not nCheckMap then
		return 1
	end
