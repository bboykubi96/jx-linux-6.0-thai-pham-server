...
NPC_INFO = 
{ 
	{1, "doctornpc", 55, "¾üÐè¹Ù(ËÎ)", "\\script\\missions\\battle\\npc\\doctor.lua"},
	{2, "doctornpc", 49, "½ð¹ú¾üÐè¹Ù", "\\script\\missions\\battle\\npc\\doctor.lua"},
{1, "symbolnpc", 629, "", "", 1}, 
{2, "symbolnpc", 630, "", "", 1}, 
	{1, "depositobj", 625, "´¢ÎïÏä", "\\script\\battles\\openbox.lua"},
	{2, "depositobj", 625, "´¢ÎïÏä", "\\script\\battles\\openbox.lua"},
} 

local Rule = Dungeon:new_type("Tèng kim ngay c¶ server") 
...
local tbInfo = 
{ 
szBattleName = " tèng kim ngay c¶ server", 
szMissionName = " thï giÕt d¹ng thøc ", 
szMissionDesc = " bèi c¶nh : c«ng nguyªn 1100 n¨m , kim chñ hoµn nhan l­îng muèn thùc hiÖn diÖt tèng ®Ých m­u ®å , tËp trung ®¹i qu©n , h­íng nam tiÕn tíi gÇn , kim qu©n dÇn dÇn x©m chiÕm ®Õn nam ph­¬ng , chiÕm lÜnh t­¬ng d­¬ng , triÓn khai mét cuéc tµn khèc chiÕn tranh , b¶o vÖ t­¬ng d­¬ng thµnh cùc kú gian khæ . <enter><enter><color=yellow>. ph­¬ng ph¸p chiÕn ®Êu # ë trªn chiÕn tr­êng , lÊy ®­îc ph¶n ®å ®Ých nhµ ch¬i sÏ lÊy ®­îc tÝch ph©n . bªn kia tÝch ph©n cao lµ chiÕn th¾ng . <enter> trªn chiÕn tr­êng kh«ng cã NPC chiÕn ®Êu .", 
szMiniMap = format("Thø %d trµng ", pBattle.nId), 
} 
...
Msg2Map(self.nMapId, format("B©y giê tèng ®Ých tÝch ph©n ®Õm lµ : %d, kim ®Ých tÝch ph©n lµ : %d", nSongPoint, nKimPoint)) 
end 
...
Msg2Map(self.nMapId,"ChiÕn ®Êu b¾t ®Çu ") 
...
local nCamp = self.pBattle:GetPlayerCamp(szName) 
if nCamp then 
LeaveChannel(PlayerIndex, format("%s trµng lÇn %d", CAMP_NAME[nCamp], self.pBattle.nId)) 
end 
...
function Rule:close() 
ClearMapTrap(self.nMapId) 
DeleteChannel(format("%s trµng lÇn %d", CAMP_NAME[CAMP_SONG], self.pBattle.nId)) 
DeleteChannel(format("%s trµng lÇn %d", CAMP_NAME[CAMP_KIM], self.pBattle.nId)) 
if self.pBattle then 
