...
BattleManagerDef = 
{ 
{"S¬ cÊp tèng kim ngay c¶ server", 972, 5000, 35000}, 
{"Trung cÊp tèng kim ngay c¶ server", 973, 30000, 50000}, 
{"Cao cÊp tèng kim ngay c¶ server", 974, 45000}, 
} 

NPC_LIST = 
{ 
	{235, "³µ·ò", {1535,3154},"\\script\\missions\\battle\\npc\\chefu.lua"},
	{55, "¾üÐè¹Ù", {1546,3159}, "\\script\\missions\\battle\\npc\\doctor.lua"},
	{62,"Ä¼±ø¹Ù", {1549,3179}, "\\script\\activitysys\\npcdailog.lua"},
} 
...
return Talk(1, "", " ë cïng trµng tèng kim chiÕn trung kh«ng thÓ ®æi tr¹i lÝnh ") 
end 
end 
local bRet = pPrevBattle:IsInPlaying(szName) 
if bRet then 
return Talk(1, "","Ng­¬i ®ang mét ng­êi kh¸c chiÕn tr­êng ") 
end 
pPrevBattle:CancelWaiting(szName) 
end 
if pBattle:JoinWaiting(szName, nCamp) then 
BattleManagerPlayer[szName] = pBattle 
-- script viet hoa By http://tranhba.com Fix l# text - Modified by DinhHQ - 20111025 
Msg2Player(format("Ng­¬i ®· thµnh c«ng ghi danh gia nhËp %s ®Ých danh s¸ch ", %CAMP_NAME[nCamp])); 
end 
...
-- script viet hoa By http://tranhba.com  local nYear = tonumber(GetLocalDate("%Y")) 
-- script viet hoa By http://tranhba.com  local nMonth = tonumber(GetLocalDate("%m")) 
-- script viet hoa By http://tranhba.com  local nDay = tonumber(GetLocalDate("%d")) 
-- script viet hoa By http://tranhba.com  local nHM = tonumber(GetLocalDate("%H%M")) 

-- script viet hoa By http://tranhba.com  local nTime = Tm2Time(nYear, nMonth, nDay, floor(START_TIME/100), mod(START_TIME,100)) 
-- script viet hoa By http://tranhba.com  local nNextTime = nTime - GetCurServerTime() 

-- script viet hoa By http://tranhba.com  if nNextTime <= 0 then 
-- script viet hoa By http://tranhba.com 		nNextTime = nNextTime + 24 * 60 * 60
