...
local szId = format("Thø %s trµng %d", CAMP_NAME[k], self.nId) 
...
-- script viet hoa By http://tranhba.com  ®Ó cho hai trËn doanh nh©n sè t­¬ng ®¼ng 
...
local szTitle = format("Thø %d trµng tèng kim b¾t ®Çu , muèn ®i vµo chiÕn tr­êng sao ", self.nId) 
local tbOpt = 
{ 
...
-- script viet hoa By http://tranhba.com  trËn doanh ghi danh nh©n sè th­îng h¹n ®Ò kú 
if (nCamp == CAMP_SONG and nCurSong >= CAMP_PLAYER_MAX_COUNT) or 
(nCamp == CAMP_KIM and nCurKim >= CAMP_PLAYER_MAX_COUNT) then 
Talk(1, "", format("TrËn doanh ghi danh nh©n sè nhiÒu nhÊt v× ?%d ng­êi , xin gia nhËp chç kh¸c hoÆc tham gia kh¸c trËn doanh .", CAMP_PLAYER_MAX_COUNT)) 
return 
end 
...
Talk(1, "", format("C¸i nµy tr¹i lÝnh qu©n ®éi ®Õm qu¸ nhiÒu ( chªnh lÖch kh«ng thÓ v­ît qua %d ng­êi ), ng­¬i cã thÓ gia nhËp kh¸c tr¹i lÝnh .", PLAYER_COUNT_DIFFERENCE)) 
return 
end 
...
-- script viet hoa By http://tranhba.com  if self.nTimerId and self.nState == SIGNUP_STATE then 
-- script viet hoa By http://tranhba.com  tbInfo.nLastTime = TimerList:GetRestTime(self.nTimerId) 
-- script viet hoa By http://tranhba.com  elseif self.pRule and self.nState == PREPARE_STATE then 
-- script viet hoa By http://tranhba.com  tbInfo.nLastTime = self.pRule.nRestTime - RUNNING_TIME 
-- script viet hoa By http://tranhba.com  elseif self.pRule and self.nState == RUN_STATE then 
-- script viet hoa By http://tranhba.com  tbInfo.nLastTime = self.pRule.nRestTime 
...
local pDungeonType = DungeonType[" tèng kim ngay c¶ server"] 
...
if nWinCamp == 0 then 
Msg2Map(self.pRule.nMapId, format("KÕt qu¶ hãa ")) 
else 
...
if nPlayerIndex > 0 then 
CallPlayerFunction(nPlayerIndex, Msg2Player,"Tèng kim ®· kÕt thóc , xin/mêi lÇn n÷a lùa chän ®éi ngò ") 
end 
end 
...
CallPlayerFunction(nPlayerIndex, Msg2Player, format("Ng­¬i lÊy ®­îc %d liªn tiÕp server cèng hiÕn .", nExtPoint)) 
...
{ 
-- script viet hoa By http://tranhba.com { tr­íc m¾t ng­êi Tèng ®Õm , tr­íc m¾t ng­êi Kim ®Õm , chê ®îi ng­êi Tèng ®Õm , chê ®îi ng­êi Kim ®Õm , kÕt qu¶ tèng , kÕt qu¶ kim , kÕt qu¶ chê tèng , kÕt qu¶ chê kim }, 
{50,30,0,0,50,30,0,0}, 
...
-- script viet hoa By http://tranhba.com TestBattleClass:RunTest() 
