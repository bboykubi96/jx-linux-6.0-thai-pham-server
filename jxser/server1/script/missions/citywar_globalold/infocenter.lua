...
function main()
	...
	dofile("script/missions/citywar_global/citywar_function.lua")
	--if(congthanhchien == 0) then
		--Say("C«ng thµnh chiÕn t¹m thêi ch­a më")
		--return 1;
	--end
	ArenaBaoDanh()				--L«i ®µi
...
function ArenaBaoDanh()
	...
	if (IsSigningUp(nCityId) == 1) then
		Say(format("HiÖn giê <color=yellow>%s<color> l«i ®µi C«ng Thµnh ChiÕn ®ang bao danh, c¸c h¹ cã muèn b¸o danh cho bang héi cña m×nh kh«ng? Tèi thø 5 18h00 ®Õn b¸o danh l«i ®µi 20h00 tham gia ®¸nh l«i ®µi, Tèi thø 6 20h00 ®Õn tham gia c«ng thµnh",GetCityAreaName(nCityId)), 
			5, 
			...
			"L·nh th­ëng c«ng thµnh chiÕn /talk_citytong_award",
...
function SignUpTheOne()
	...
	if (nWeek ~= 4 ) then
		Say("Tèi thø 5  18h00 ®Õn b¸o danh l«i ®µi 20h00 tham gia ®¸nh l«i ®µi")
		return 1
	end
	
	if kiemtrabangchu() == 0 then
		Say("ChØ cã <color=red>Bang Chñ<color> míi ®ñ t­ c¸ch b¸o danh")
		return 1
	end
