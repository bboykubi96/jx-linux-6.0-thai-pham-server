...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function XemDiem()
Say("<bclr=violet>Sè §iÓm PK Trong B¶o Tµng Viªm §Õ Cña C¸c H¹ Lµ:<bclr=red>"..GetTask(1851).."<bclr>\nQuy Tr×nh §æi: 1 §iÓm = 3 Xu\n<bclr=blue>L­u ý: Khi Tham Gia B¶o Tµng Viªm §Õ, Toµn Bé §iÓm\nPK SÏ TÝnh L¹i Tõ §Çu, Nªn Khi C¸c Nh©n SÜ Tham Gia\nB¶o Tµng Viªm §Õ, NÕu Cã §iÓm H·y §æi §iÓm Tr­íc Khi Tham Gia B¶o Tµng Viªm §Õ.<bclr>\n             Chóc C¸c Nh©n SÜ Ch¬i Games Vui VÎ...!",2,"Ta Muèn §æi/DoiDiem","Hñy Bá/No<bclr>")
end

function DoiDiem()
	if GetTask(YDBZ_TEAMS_TASKID) < 1 then
		Talk(1,"","<bclr=red>C¸c H¹ Ch­a §ñ §iÒu KiÖn NhËn Th­ëng H·y Cè G¾ng H¬n N÷a")
	else
		AskClientForNumber("DoiThuong",0,100,"NhËp Sè L­îng")
	end
end

function DoiThuong(num)
local nPK = GetTask(YDBZ_TEAMS_TASKID) nPK_SL = num*1 Xu = num*3
	if (nPK >= nPK_SL) then
		for i = 1,nPK_SL do
			SetTask(YDBZ_TEAMS_TASKID,GetTask(YDBZ_TEAMS_TASKID)-1)
		end
	for i = 1,num do
		AddStackItem(3,4,417,1,1,0,0)
	end
		Msg2Player("Chóc Mõng C¸c H¹ §· §æi Thµnh C«ng <color=yellow>"..num.."<color> §iÓm PK Viªm §Õ, NhËn §­îc <color=yellow>"..Xu.."<color> V¹n Ng©n L­îng")
	else
		Talk(1,"","<bclr=red>C¸c H¹ Kh«ng §ñ "..nPK_SL.." §iÓm, Cßn ThiÕu "..nPK_SL-nPK.." §iÓm")
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function YDBZ_about()
	...
	Say(format("<bclr=violet>HiÖn t¹i B¶o Tµng Viªm §Õ Vâ L©m TruyÒn Kú Më full time! C¸c Nh©n SÜ Vâ L©m TruyÒn Kú Cã ThÓ Tho¶i M¸i PK! <enter>Mçi Ngµy %s, H·y §Õn Tr­íc 5 Phót §Ó B¸o Danh ë B×nh B×nh C« N­¬ng", "<bclr=red> 00:25 01:25 02:25 03:25 04:25 07:25 08:25 09:25 10:25 11:25 12:25 13:25 14:25 16:25 18:25 22:25<bclr>"),0)
