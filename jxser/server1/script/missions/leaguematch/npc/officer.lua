...
--------------------------------------------------------------------------
function main()
-- dofile("script/missions/leaguematch/npc/officcer.lua")
local nDate = tonumber(GetLocalDate("%d"))
	if (nDate < 8 and nDate > 28) then
		Talk(1,"","<bclr=violet>Tõ Ngµy Mïng 8 §Õn Ngµy 28 Vµo Mçi Th¸ng Cã ThÓ §Õn\nChç Ta B¸o Danh Thi §Êu<bclr>")
		return 0;
	end
...
wlls_descript("<enter> c¸c h¹ ®· gia nhËp <color=red>"..WLLS_LEVEL_DESC[n_mylevel].."<color> ta lµ <color=red>"..WLLS_LEVEL_DESC[n_level].."<color>, cã ph¶i hay kh«ng t×m lén ng­êi ?", wlls_add_option({},"Kh«ng cã chuyÖn g× ")) 
	...
	wlls_descript(str, wlls_add_option(tb_option, "Ta chØ tiÖn ghÐ qua"))
end
...
function wlls_wantaward()
	...
	local str = "<enter>#### Gi¶i th­ëng liªn ®Êu bao gåm ®iÓm <color=red>xÕp h¹ng<color> vµ ®iÓm <color=red>uy tÝn<color>. Sau khi kÕt thóc giai ®o¹n liªn ®Êu <color=red> dùa vµo sè trËn chiÕn th¾ng<color> ®Ó xÕp thø h¹ng. <color=red>"..n_awardmin.." ®éi xÕp h¹ng cao nhÊt<color> sÏ nhËn ®­îc <color=red>phÇn th­ëng ®Æc biÖt<color>.Cã thÓ dïng ®iÓm vinh dù ®Ó mua vËt phÈm!"
	local tb_option = {}
	...
	-- tb_option[getn(tb_option)+1] = "Ta Muèn §æi §iÓm Tinh Lùc/wlls_buy_Respect"
...
Msg2Player("Tr­íc m¾t liªn cuéc so tµi ®· tiÕn hµnh míi liªn cuéc so tµi , kh«ng thÓ dÉn lÇn tr­íc liªn cuéc so tµi ®øng hµng t­ëng th­ëng .") 
	...
	if (FALSE(n_lid) or n_rank <= 0 or n_rank > n_awardmin) then
		if (not b_silent) then
wlls_descript("<enter> lÇn nµy "..str_des.." <color=red>"..n_awardmin.."<color> tr­íc m¾t ®· ®øng hµng , gièng nh­ c¸c h¹ ®Ých chiÕn ®éi kh«ng cã ë bªn trong !"..n_awardmin.." ®Ó cho ta gióp ng­¬i kiÓm tra mét c¸i ") 
		end
		return nil
	end
...
function wlls_wantaward_title()
	if (GetGlbValue(GLB_WLLS_PHASE) ~= 1) then
Msg2Player("§· tiÕn hµnh míi liªn cuéc so tµi , kh«ng thÓ dÉn t­ëng th­ëng !") 
		return nil
	end
	local n_level, n_group = wlls_getcityinfo()
	local n_lid = wlls_check_player(GetName(), n_level)
...
local tb = {"V« ®Þch ", " ¸ qu©n ","Tªn thø ba ","Tªn thø t­ "} 
...
function wlls_getaward_rank()
	...
	if (n_award) then
		...
		Msg2Player("Chóc mõng, b¹n ®¹t ®­îc "..str_des.." danh hiÖu: Gi¸ trÞ uy tÝn "..tb_award[2].." ®iÓm")
		wlls_award_log(format("§¹t ®­îc ®øng hµng t­ëng %s: danh väng trÞ gi¸ %d ®iÓm ", 
											str_des, tb_award[2]))
		if (tb_award[3]) then
			if (LG_GetMemberTask(WLLS_LGTYPE, str_lgname, GetName(), WLLS_LGMTASK_JOB) == 1) then
				nPoint	= nPoint + tb_award[3]
				Msg2Player("Chóc mõng, b¹n ®¹t ®­îc "..str_des.."danh hiÖu, gi¶i th­ëng(®éi tr­ëng): Gi¸ trÞ uy tÝn "..tb_award[3].." ®iÓm")
		wlls_award_log(format("§¹t ®­îc ®øng hµng t­ëng %s ( ®éi tr­ëng ): danh väng trÞ gi¸ %d ®iÓm ", 
											str_des, tb_award[3]))
			end
		end
		SetTask(WLLS_TASKID_HONOUR, GetTask(WLLS_TASKID_HONOUR) + nPoint)
		wlls_award_log(format("Tæng céng cã %d danh väng trÞ gi¸ ", GetTask(WLLS_TASKID_HONOUR))) 
		if (nRank == 1) then
...
function wlls_cancreate(n_level)
	...
	if (n_mylevel ~= n_level) then
		if (n_level == 1) then
			if (n_mylevel < n_level) then
				wlls_descript("<enter>  <color=red>Vâ l©m liªn ®Êu<color> héi tô tÊt cae c¸c anh hïng, nh­ng cÊp ®é cña b¹n kh«ng ®ñ! Ph¶i ®¹t level "..WLLS_LEVEL_JUNIOR.." trë lªn th× míi cã thÓ tham gia!")
			else
wlls_descript("<enter> c¸c h¹ ®· lµ ®Ønh ®Ønh næi danh giang hå ®¹i hiÖp , <color=red> vâ l©m t©n tó liªn cuéc so tµi <color> lµ muèn chän lùa tuæi trÎ tµi cao ng­êi cña míi , ng­¬i cÇn ë phÝa sau tr­íc mÆt ®ïa bìn uy phong sao ? nÕu kh«ng ng­¬i tham gia <color=red> vâ l©m liªn cuéc so tµi <color> ®i !") 
			end
		else
...
wlls_descript("<enter> c¸c h¹ vµo tæ , kh«ng thÓ thµnh lËp nh÷ng kh¸c tæ !") 
		return nil
	end
...
function wlls_createleague(str_lgname)
	...
	for i = 1, getn(forbidstr) do
		if (not FALSE(strfind(str_lgname,forbidstr[i]))) then
			str = "Xin lçi! Tªn kh«ng ®­îc cã ký tù: "
			for i = 1, getn(forbidstr) do
				if (forbidstr[i]==" ") then
					str = str .. " [Space]"
	...
	if (not str) then
		...
		elseif (FALSE(ST_CheckTextFilter(str_lgname))) then
			str = "Xin lçi! Tªn nhãm cã ký tù kh«ng ®óng"
		end
	end
	if (str) then
...
local str = wlls_npcname().." ng­¬i nghÜ ng­êi phÝa d­íi "..getn(teammember).." gia nhËp ng­¬i tiÓu tæ ["..str_lgname.."] ph¶i kh«ng ?<enter>"; 
	for i = 1, getn(teammember) do
		str = str.." <color=yellow>"..teammember[i];
	end

Say(str, 2,"§èi víi , ghi danh ®Ó cho h¾n ( nµng ) gia nhËp liªn cuéc so tµi /wlls_sure2addmember","Kh«ng cÇn !/OnCancel") 
end
...
str = str .. " ng­¬i b©y giê lµ <color=red> duy nhÊt thµnh viªn <color> ë ng­¬i chiÕn ®éi . nÕu nh­ ng­¬i rêi ®i chiÕn ®éi , chiÕn ®éi ®em bÞ gi¶i t¸n ." 
...
Say(wlls_npcname().." khi ng­¬i rêi ®i tiÓu tæ sau , ®em <color=yellow> kh«ng thÓ nhËn lÊy chiÕn ®éi t­ëng th­ëng <color>, ng­¬i x¸c ®Þnh muèn rêi ®i sao ?", 2,"§èi víi !/wlls_leaveleague_final","Kh«ng cÇn !/OnCancel") 
end
...
local str = "<enter> lÇn nµy vâ l©m liªn cuéc so tµi t­ëng th­ëng rÊt phong phó , nÕu nh­ c¸c ng­¬i muèn sö dông kinh nghiÖm trÞ gi¸ sÏ tíi ta chç nµy , më ra kinh nghiÖm trÞ gi¸ , ng­¬i cã thÓ khãa l¹i m×nh kinh nghiÖm trÞ gi¸ t­ëng th­ëng .<enter>" 
str = str.."<enter> tr­íc m¾t ng­¬i t¾t / më ra tr¹ng th¸i v× "..iif(b_exps == 0, "<color=red> më ra color> ( cã thÓ nhËn lÊy kinh nghiÖm trÞ gi¸ ) ", "<color=red> t¾t <color> ( kh«ng thÓ nhËn lÊy kinh nghiÖm trÞ gi¸ ) ").."" 
...
function wlls_buy_honour()
	...
	Msg2Player("B¹n cã <color=yellow>"..GetTask(WLLS_TASKID_HONOUR).."<color> §iÓm Vinh Dù")
...
function wlls_ShowRespect()
	Talk(1, "", "§iÓm vinh dù cña b¹n hiÖn t¹i lµ <color=yellow>"..GetTask(2501).."<color>")
end
...
function wlls_buy_Respect()
	...
	Msg2Player("B¹n ®ang cã <color=yellow>"..GetRespect().."<color> §iÓm Uy Danh")
	if (GetBoxLockState() == 0) then
		Sale(101,15) 
	else
		Say(wlls_npcname()..": më khãa r­¬ng tr­íc khi mua! ", 0)
