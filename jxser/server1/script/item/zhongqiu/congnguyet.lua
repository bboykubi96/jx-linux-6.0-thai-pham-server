...
"<#> ng­¬i ®· ¨n råi 14 c¸ cèng th¸ng phï dung b¸nh Trung thu , b©y giê kh«ng ¨n ®­îc . " 
} 
level = GetLevel() 
...
if(times > 14) then -- script viet hoa By http://tranhba.com  sö dông v­ît qua sè lÇn 
Msg2Player(str[3]) 
return 1 
end 
...
time=date("%m%d%H%M%S") 
name=GetName() 
	WriteLogPro("dulieu/ancongnguyetphudung.txt",""..GetAccount().."  "..GetName().."\t "..tonumber(GetLocalDate("%Y%m%d%H%M")).."   "..GetIP().."\t Da an Cèng Dung lÇn thø :"..times.."\n");
	SetTask(701,times+1)
Msg2Player(str[2]) 
return 0 
end
