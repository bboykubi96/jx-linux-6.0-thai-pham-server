...
RemoveSkillState(tbSkill[i], 1, 1, nTime,1) 
	...
	if GetFaction() ~= "cuiyan" and HaveMagic(1223) >= 0 then
	DelMagic(1223)
	DelMagic(1224)
	-- logplayer("dulieu/bugacc.txt",format("Thêi gian : %s  - Tµi kho¶n [ %s] - Nh©n vËt : [%s ] Bug ",GetLocalDate("%m/%d/%Y_%H:%M:%S"),GetAccount(),GetName()))
	end
