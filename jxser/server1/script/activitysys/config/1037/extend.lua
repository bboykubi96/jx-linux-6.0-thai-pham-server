...
function pActivity:GiveTaskNowLTT()
	...
	local szNpcName = format("%s Xe Ngùa Chë N­íc", szName)

	DynamicExecute("\\script\\activitysys\\config\\1037\\carriage.lua", "add_carriage_ltt", nMapId, pTask.nId, szNpcName, GetCurCamp())
...
function pActivity:ServerStart()
	...
	for i=1,getn(%MOSTER_POS_LTT) do
		local nIndex = random(1, 3)
		NpcFunLib:AddFightNpc({szName="GiÆc Cá",nNpcId=tbNpcId[nIndex],nLevel=95,bNoRevive=0},{%MOSTER_POS_LTT[i]})
	end
...
function pActivity:StartLoaderThonLangLTT()
