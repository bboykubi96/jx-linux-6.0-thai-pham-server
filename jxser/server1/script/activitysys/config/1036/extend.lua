...
function pActivity:GiveTaskNowVLT()
	...
	DynamicExecute("\\script\\activitysys\\config\\1036\\carriage.lua", "add_carriage_vlt", nMapId, pTask.nId, szNpcName, GetCurCamp())
...
function pActivity:ServerStart()
	...
	for i=1,getn(%MOSTER_POS_VLT) do
		local nIndex = random(1, 3)
		NpcFunLib:AddFightNpc({szName="GiÆc Cá",nNpcId=tbNpcId[nIndex],nLevel=95,bNoRevive=0},{%MOSTER_POS_VLT[i]})
	end
...
function pActivity:StartLoaderThonLangVLT()
