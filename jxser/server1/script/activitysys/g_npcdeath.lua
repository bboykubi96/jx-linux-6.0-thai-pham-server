...
function OnGlobalNpcDeath(nNpcIndex, nAttackerIndex)
	if PlayerIndex and PlayerIndex > 0 then
		...
		-- if NPCINFO_GetLevel(nNpcIndex) >= 40 and random(1000) < 15 then
		-- tbDropTemplet:GiveAwardByList(nNpcIndex, PlayerIndex, {{tbProp={6,1,4391,1,0,0},nExpiredTime=20180328}}, "Drop moi fix", 1)
		-- end
		...
		-- end
		-- if NPCINFO_GetLevel(nNpcIndex) > 90 then
		-- local nTeamSize = GetTeamSize()
