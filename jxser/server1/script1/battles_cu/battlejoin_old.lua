...
function main()
	...
	--if GetLevel() < 80 then
	--	Say("§¹i hiÖp ®¹t ®¼ng cÊp 80 trë lªn míi cã thÓ tham gia")
	--	return 1
	--end

	if thamgiatongkim == 0 then
		Say("Chøc n¨ng Tèng Kim t¹m thêi ch­a më")
		return 1
	end
...
if (BT_GetGameData(GAME_BATTLEID) ~= BT_GetData(PL_BATTLEID) or BT_GetGameData(GAME_BATTLESERIES) ~= BT_GetData(PL_BATTLESERIES)) then
	...
	if (bt_ncamp == 1) then
		Say("ChiÕn dŞch ["..battlename.."] ChiÕn dŞch ®· b¾t ®Çu, mäi ng­êi h·y v× tö d©n ®¹i tèng, tôc ng÷ cã c©u: Thiªn h¹ h­ng vong, thÊt phu h÷u tr¸ch. Nay ng­êi kim x©m ph¹m bê câi chóng ta, ®©y lµ lóc b¸o hiÕu ®Êt n­íc, chØ cÇn cÊp 40 trë lªn vµ cÇn 20 v¹n l­îng th× cã thÓ ®Òn ®¸p quèc gia råi, nµo ng­êi anh em cßn chÇn chõ g× n÷a!", 2, "Ta tham gia! (§iÓm tİch lòy trë vÒ 0) /bt_joinsong", "§Ó ta suy nghÜ l¹i!/bt_oncancel");
	else
		Say("ChiÕn dŞch ["..battlename.."] ChiÕn dŞch ®· b¾t ®Çu, hìi c¸c vŞ dòng sÜ Kim quèc, thêi kh¾c nhÊt thèng thiªn h¹ vµ dÑp bän Tèng quèc nam man c¶n ®­êng cña chóng ta ®· ®Õn. Nay Kim quèc rÊt cÇn sù trî lùc cña c¸c ng­¬i, chØ cÇn cÊp 40 trë lªn vµ cÇn 20 v¹n l­îng lµ cã thÓ ®Òn ®¸p quèc gia råi, nµo ng­êi anh em cßn chÇn chõ g× n÷a!", 2, "Ta tham gia! (§iÓm tİch lòy trë vÒ 0) /bt_joinjin", "§Ó ta suy nghÜ l¹i!/bt_oncancel");
	end
	SubWorld = nOldSubWorld
	return	
end;
...
function bt_enterbattle()
	...
	--Update G7VN set chien truong cao cap
	MapId = 325
	if (MapId > 0) then
		if (idx < 0) then
