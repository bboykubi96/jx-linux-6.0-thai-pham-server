...
function nw_enthrone()
	...
	if (result == 0 and NW_GetEmperorName() == "") then
		Say("VÞ anh hïng nµy cã chÝ khÝ h¬n ng­êi, nh­ng h·y ®îi thêi c¬ ®·.")
...
-- Á?È¡
function nw_accept()
	Say("Tr¸ng sü xuÊt trËn, giÕt ®Þch lËp c«ng",
		3,
...
SetTask(3017, nDate); --set ngay 6
	...
	local szNews = format("Quèc Chñ  <color=green>"..GetName().."<color=yellow> ®· nhËn thµnh c«ng 55 quèc chiÕn lÖnh bµi chuÈn bÞ cho ®¹i chiÕn Thiªn Tö s¾p tíi.");
...
function nw_accept_domainring()
	...
	if nWiner ~= 1 and nWiner ~= 2 then
		Say("VÉn ch­a ®Þnh ®o¹t ®­îc ai lµ thiªn tö")
		return
	end

	if nWiner == 1 and city ~= CITYIDX_BIANJING then
		Say("PhÇn th­ëng dµnh cho ng­êi cã hïng t©m tr¸ng chê dµnh l¹i giang s¬n, xin h·y ®Õn thµnh thÞ cña m×nh ®Ó lÜnh th­ëng!")
		return
	end
	...
	elseif (GetTong() ~= tong) then
		Say("Nh×n bé d¹ng lËp lê cña ng­êi, h¼n lµ lµ gian tÕ cña ®Þch?")
...
function nw_comment()
	Say("Tuy r»ng quyÒn thÕ cña Thiªn Tö cao nh­ng còng kh«ng thÓ xem th­êng lßng d©n ®­îc.",
		3,
		"NhËn xÐt ®­¬ng kim Thiªn Tö/nw_comment_commit",
		"Xem kÕt qu¶ nhËn xÐt/nw_comment_query",
		"Kh«ng cã viÖc g×/Cancel")
end

-- ?ú±ÊÖ±Êé
function nw_history()
	Say("Hµo kiÖt tuy nhiÒu, nh­ng trong lÞch sö chØ l­u danh, tõ cæ chÝ kim cã mÊy ng­êi?",
		4,
		"LÞch sö thiªn tö/nw_history_emperor",
		"Tèng quèc sö ký/nw_history_song",
		"Kim quèc sö ký/nw_history_jin",
...
function nw_oninputnationtitle(title)
	...
	elseif (len > NW_MAXLEN_NATIONTITLE) then
		Say("BÈm bÖ h¹! Quèc hiÖu nµy qu¸ dµi råi, b¸ch tÝnh kh«ng thÓ nhí ®­îc ®©u.",
			2,
...
function nw_emperororder_congratulate()
	...
	if (time <= NW_GetTask(NW_TASKNO_CONGRATULATION) + SECONDS_ONEWEEK) then
		Say("BÖ h¹, lÔ nghi qu¸ nhiÒu sÏ rÊt hao tµi tèn cña mµ kh«ng hîp lý.")
	else
...
function nw_emperororder_congratulate_words(words)
	...
	elseif (len > NW_MAXLEN_CONGRATULATIONWORDS) then
		Say("Lêi chóc dµi qu¸ sÏ lµm b¸ch tÝnh rÊt khã nhí.")
...
function nw_emperororder_levy()
	...
	if (time <= NW_GetTask(NW_TASKNO_LEVY) + SECONDS_ONEWEEK) then
		...
		NW_Levy(10000, 80)
		AddGlobalCountNews("[Thiªn Tö quyªn tiÒn] Thiªn Tö cã chØ tÊt c¶ thÇn d©n quyªn tiÒn, kh«ng ai ®­îc phÐp kh¸ng chØ.", 3)
	end
end
...
function nw_emperororder_message()
	...
	if (tasktime ~= 0 and currtime <= tasktime + SECONDS_ONEWEEK and
		...
		Say("BÖ h¹ ph¸t qu¸ nhiÒu chiÕu th­, b¸ch tÝnh ®Òu kh«ng muèn nghe.")
	else
...
function nw_emperororder_onmessage(msg)
	...
	elseif (NW_CheckText(msg) ~= 1) then
		Say("ChiÕu th­ cã mét sè tõ kh«ng æn, xin bÖ h¹ h·y suy xÐt l¹i.",
			2,
			"Xin mêi nhËp l¹i/nw_emperororder_inputmessage",
			"§Ó ta suy nghÜ l¹i/Cancel")
	else
...
function nw_emperororder_forbidtalkdlg()
	...
	if (tasktime ~= 0 and currtime < tasktime + SECONDS_ONEDAY) then
		...
		Say("MÆc dï bÖ h¹ cã thÓ cÊm nãi mét ng­êi, nh­ng lßng ng­êi cã thÓ luËn ®o¸n. Xin bÖ h¹ h·y thËn träng khi hµnh sù",
			2,
			"Ta ®· quyÕt ®Þnh råi/nw_emperororder_forbidtalk",
			"§Ó ta suy nghÜ l¹i/Cancel")
	end
end
...
function nw_emperororder_forbidtalk_input(name)
	...
	AddGlobalCountNews(format("[ChiÕu th­ thiªn h¹]\"<color=red>%s<color>\"Chäc gi©n Thiªn Tö, cÊm nãi nöa giê thÞ uy.", name), 3)
...
function nw_comment_query()
	local emperor = NW_GetEmperorName()
	if (emperor == "") then
		Say("HiÖn t¹i vÉn ch­a cã thiªn tö, b¸ch tÝnh rÊt chê mong mét minh qu©n thèng nhÊt thiªn h¹")
		return
	end
	...
	Say(format("§­¬ng kim Thiªn Tö lµ <color=yellow>%s<color>, thiÖn ¸c lµ <color=yellow>%d<color>, vÒ phÝa <color=yellow>%s<color>.", emperor, value, title))
end
