-- script viet hoa By http://tranhba.com  t©y nam nam khu thóy khãi cöa tr÷ vËt r­¬ng 2 

function main(sel) 
OpenBox(); -- script viet hoa By http://tranhba.com  b¾n ra tr÷ vËt r­¬ng vËt phÈm khu«ng cïng nhµ ch¬i vËt phÈm khu«ng , ®Ó víi nhµ ch¬i tiÕn hµnh vËt phÈm cïng kim tiÒn ®İch tån lÊy 
if (GetFaction() == "cuiyan") then 
SetRevPos(62); -- script viet hoa By http://tranhba.com  thiÕt trİ sèng l¹i ®iÓm 
end 
end;
