...
local nDate = tonumber(GetLocalDate("%d"))
	...
	if GetLevel()<80 then
		Say("Nh©n vËt trªn cÊp 80 míi cã thÓ sö dông.")
		return 1
	end
	if (GetTask(NHANTHUONG) < 5) then
	AddOwnExp(3e6)
	--	tbAwardTemplet:GiveAwardByList({{nExp_tl = 6000000}}, "test", 1);
	...
	--	tbAwardTemplet:GiveAwardByList({{szName = "Bã Cá",tbProp={6,1,4415,1,1},nCount=1},}, "test", 1);
			...
			--	tbAwardTemplet:GiveAwardByList({{szName = "TiÒn §ång",tbProp={4,417,1,1},nCount=2},}, "test", 1);
--	tbAwardTemplet:GiveAwardByList({{szName = "Tinh ngÃ¤c",tbProp={6,1,4409,1,1,0},nCount=20},}, "test", 1);
		...
		Say("Mçi Ngµy ChØ ¡n §­îc 5 Qu¶ Th«i!")
		return 1;
	end
