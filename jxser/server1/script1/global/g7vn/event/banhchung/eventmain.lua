...
----------------------------------------
function main()
dofile("script/global/g7vn/event/banhchung/eventmain.lua")
...
--	if GetTask(5334)>=10 then
	...
	--	tbLogEvent:SapXepDSXepHang()
		...
		--Msg2Player(""..check.."")
		--tbLogEvent:SapXepDSXepHangdanhan(1)
--end
	local tbDailog = DailogClass:new(szNpcName);
	tbDailog.szTitleMsg = "<npc>§¹i HiÖp <color=yellow>"..GetName().."<color> ®· sö dông <color=red>"..GetTask(4997).."<color> <color=cyan>B¸nh Ch­ng Th­îng H¹ng<color>",
	G_ACTIVITY:OnMessage("ClickNpc", tbDailog, nNpcIndex)
EventSys:GetType("AddNpcOption"):OnEvent(szNpcName, tbDailog, nNpcIndex)
	local nDate = tonumber(GetLocalDate("%Y%m%d"));
	local nTime = tonumber(GetLocalDate("%H%M"));
	if (nDate >= startdate and nDate <= endate) then
...
--	tbDailog:AddOptEntry("Mua Hép Diªm (5xu-1hép)", muahopdiem);
	...
	if GetAccount()=="thaipham1" or GetAccount()=="testgame4" then
		...
		tbDailog:AddOptEntry("Mua 100 B¸nh Ch­ng Th­îng H¹ng (1300 Xu)", MuaEvent500xu);
			tbDailog:AddOptEntry("Mua ThÞt Heo (10 Xu - 1 C¸i)", BatDauMuaHoa);
			
	 tbDailog:AddOptEntry("NÊu B¸nh Ch­ng Th­îng H¹ng (Event Xu)", GhepBoHoaBonMua);
	 tbDailog:AddOptEntry("Mua §Ëu Xanh (3 V¹n - 1 C¸i)", BatDauMuaHoa2);
	  tbDailog:AddOptEntry("NÊu B¸nh Ch­ng Th­êng (Event V¹n)", GhepBoHoaBonMua2);
	tbDailog:AddOptEntry("NhËn Th­ëng Mèc Event Xu", Moc2000);	
--	tbDailog:AddOptEntry("Show Top Event", ShowTopThiep);
--	tbDailog:AddOptEntry("Reset Mèc Event B¸nh Ngät (20xu)", Reset);
tbDailog:Show();
...
local tbItem = {
	{szName="B¸nh Ch­ng Th­îng H¹ng", tbProp={6,1,1662,1,0,0},nCount = 100},
}
	local nCount_muaban = CalcEquiproomItemCount(4,417,1,1)
	if  nCount_muaban >= 1300  then
		ConsumeEquiproomItem(1300, 4,417, 1,-1)
		tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn Th­ëng");
		Msg2Player("Chóc Mõng "..myplayersex().." §· Mua Thµnh C«ng 100 B¸nh Ch­ng Th­îng H¹ng")
...
local phaotieu= CalcEquiproomItemCount(6,1,1653,-1)
local phaodai= CalcEquiproomItemCount(6,1,1654,-1)
local hoavang= CalcEquiproomItemCount(6,1,1655,-1)
local tien= CalcEquiproomItemCount(4,417,1,-1)
local szTitle = "B¹n §ang Cã : \n<color=green>"..phaotieu.." <color>L¸ B¸nh \n<color=green>"..phaodai.."<color> G¹o NÕp\n<color=green>"..hoavang.."<color> §Ëu Xanh\n<color=yellow>3 Nguyªn LiÖu= 1 Bã B¸nh Ch­ng Th­êng<color>"
	local Opt = 
	{	
		{"Ta Muèn B¾t §Çu NÊu B¸nh", BatDauGhepHoa2},
...
local phaotieu= CalcEquiproomItemCount(6,1,1653,-1)
local phaotrung= CalcEquiproomItemCount(6,1,1654,-1)
local phaodai= CalcEquiproomItemCount(6,1,1656,-1)
local tien= CalcEquiproomItemCount(4,417,1,-1)
local szTitle = "B¹n §ang Cã : \n<color=green>"..phaotieu.." <color>L¸ B¸nh \n<color=green>"..phaotrung.." <color>G¹o NÕp\n<color=green>"..phaodai.."<color> ThÞt Heo\n<color=yellow>3 Lo¹i Nguyªn LiÖu= 1 B¸nh Ch­ng Th­îng H¹ng<color>"
	local Opt = 
	{	
		{"Ta Muèn B¾t §Çu NÊu B¸nh", BatDauGhepHoa},
		{"KÕt thóc ®èi tho¹i. ", End}
	}
	CreateNewSayEx(szTitle,Opt)
end
...
function BatDauGhepHoa_2(nNum)
	local tbItem = 
	{	
		{szName = "Bã B¸nh Ch­ng Th­êng", tbProp = {6, 1, 1664, 1,0,0}, nCount = nNum},	
	}
...
if phaotieu>=nNum  and phaodai>=nNum and hoavang>=nNum then
--Msg2Player(""..hoamai..","..hoaphuong..","..nNum.."")
--ConsumeEquiproomItem(nNum, 4,417, 1,-1)
ConsumeEquiproomItem(nNum, 6,1, 1653,-1)
ConsumeEquiproomItem(nNum, 6,1, 1654,-1)
ConsumeEquiproomItem(nNum, 6,1, 1655,-1)
Msg2Player("<color=green>Chóc mõng <color=yellow>"..GetName().."<color> ®· dïng nguyªn liÖu ghÐp thµnh <color=cyan> "..nNum.." Bã B¸nh Ch­ng Th­êng.")
tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn Th­ëng");
...
function BatDauGhepHoa_1(nNum)
	local tbItem = 
	{	
		{szName = "B¸nh Ch­ng Th­îng H¹ng", tbProp = {6, 1, 1662, 1,0,0}, nCount = nNum},	
	}
...
if phaotieu>=nNum and phaotrung>=nNum and phaodai>=nNum then
--Msg2Player(""..hoamai..","..hoaphuong..","..nNum.."")
--ConsumeEquiproomItem(nNum, 4,417, 1,-1)
ConsumeEquiproomItem(nNum, 6,1, 1653,-1)
ConsumeEquiproomItem(nNum, 6,1, 1654,-1)
ConsumeEquiproomItem(nNum, 6,1, 1656,-1)
Msg2Player("<color=green>Chóc mõng <color=yellow>"..GetName().."<color> ®· dïng nguyªn liÖu ghÐp thµnh <color=cyan> "..nNum.." B¸nh Ch­ng Th­îng H¹ng.")
tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn Th­ëng");
...
local tbItem = {
	{szName="§Ëu Xanh", tbProp={6,1,1655,1,0,0},nCount = nNum},
}
	local nCount_muaban = GetCash()
	if  nCount_muaban >= nNum*30000  then
		Pay(nNum*30000)
		tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn Th­ëng");
		Msg2Player("Chóc mõng "..myplayersex().." ®· mua thµnh c«ng <color=cyan>"..nNum.." §Ëu Xanh")
	else
...
local tbItem = {
	{szName="ThÞt Heo", tbProp={6,1,1656,1,0,0},nCount = nNum},
}
	local nCount_muaban = CalcEquiproomItemCount(4,417,1,1)
	if  nCount_muaban >= nNum*10  then
		ConsumeEquiproomItem(nNum*10, 4,417, 1,-1)
		tbAwardTemplet:GiveAwardByList(tbItem, "PhÇn Th­ëng");
		Msg2Player("Chóc mõng "..myplayersex().." ®· mua thµnh c«ng <color=cyan>"..nNum.." ThÞt Heo")
	else
		Say("Kh«ng ®ñ  Xu, Xin Xem L¹i")
	end
...
function Moc2000()
	...
	if GetTask(HOAPHUONGTRANG)>=600 and GetTask(NHANTHUONGMOC)==0 then
		local tbVnNewItemDropAward = {
	[1] = {
		--{szName="Ng©n L­îng",nJxb=20000000,nCount=1},

		{szName="LÖnh Bµi Gäi Boss",tbProp={6,1,4459,1,0,0},nCount=30},
				{szName="TÈy Tñy Kinh",tbProp={6,1,22,1,1},nCount=5},
		{szName="Vâ L©m MËt TÞch",tbProp={6,1,26,1,1},nCount=5},
	--	{szName="NhÊt Kû 7 Ngµy",tbProp={6,1,4363,1,1},nCount=1,nExpiredTime=7*24*60},
	--	{szName="LÔ Bao Trang Søc",tbProp={6,1,30503,1,1},nCount=1,nExpiredTime=10*24*60},
	...
	--	{szName="HuyÒn Tinh Kho¸ng Th¹ch",tbProp={6,1,147,5,0},nCount=1},
	{szName = "HuyÒn Tinh Kho¸ng Th¹ch",tbProp={6,1,147,5,0},nCount=10},
		{szName="Ngùa 8x",tbProp={0,10,5,random(5,9),0,0,0},nCount=1},
	},
	[2] = {
	--{szName="Cµn kh«n song tuyÖt béi",tbProp={6,1,4377,1,1},nCount=1,nRate=33,nExpiredTime=7*24*60},
	{szName="BÝ KÝp Kü N¨ng 9x",tbProp={6,1,2426,1,1},nCount=1,nRate=20},
		{szName="§¹i Thµnh Kü N¨ng 9x",tbProp={6,1,2424,1,1},nCount=1,nRate=20},
		{szName="NhÊt Kû 7 Ngµy",tbProp={6,1,4363,1,1},nCount=1,nExpiredTime=7*24*60,nRate=30},

		{szName="Tiªn Th¶o Lé §Æt BiÖt",tbProp={6,1,1181,1,1},nCount=1,nRate=30},
	},
	}
		tbAwardTemplet:GiveAwardByList(tbVnNewItemDropAward,"PhÇn Th­ëng Mèc 600")
		SetTask(NHANTHUONGMOC,1)
	else
		Say("§¹i hiÖp ch­a ¨n ®ñ 600 B¸nh Ch­ng Th­îng H¹ng hoÆc ®· nhËn th­ëng mèc.")
		return
	end
end
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(HOAPHUONGTRANG) >= 500) then
		if (Xu >= 20) then
			SetTask(HOAPHUONGTRANG,0) ConsumeEquiproomItem(20,4,417,1,-1) SetTask(NHANTHUONGMOC,0) 
			Talk(1,"","<color=red>"..myplayersex().." Chóc Mõng B¹n §· Reset Thµnh C«ng...!")
	else
		Talk(1,"","<color=red>"..myplayersex().." Kh«ng §ñ 20 Xu Xin KiÓm Tra L¹i...!")
		end
	else
		Talk(1,"","<color=red>"..myplayersex().." Ch­a ¨n ®ñ 500 <color=yellow>B¸nh Ngät")
	end
end
-----------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------
