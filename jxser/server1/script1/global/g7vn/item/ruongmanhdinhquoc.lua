...
function main()
dofile("script/global/g7vn/item/ruongmanhdinhquoc.lua")
if CalcFreeItemCellCount() < 5 then
Say("Hµnh trang kh«ng ®ñ 5 « trèng.")
end
local rannn=random(1,100)
if rannn<=5 then
	local aa=random(943,946)
	tbAwardTemplet:GiveAwardByList({{szName = "M¶nh §Þnh Quèc",tbProp={4,aa,1,1,0},nCount=1},}, "test", 1);
else
	local aa=random(927,942)
	tbAwardTemplet:GiveAwardByList({{szName = "M¶nh §Þnh Quèc",tbProp={4,aa,1,1,0},nCount=1},}, "test", 1);
end
end
