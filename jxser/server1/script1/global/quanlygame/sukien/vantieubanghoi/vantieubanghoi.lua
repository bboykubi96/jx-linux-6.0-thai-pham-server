...
NPC_Clickdb = {
	{625,80,4,53,1468,3286,0,"NhËn Tiªu Bang Héi",0,"\\script\\global\\quanlygame\\sukien\\vantieubanghoi\\vantieubanghoi.lua","main", 0},   
}
...
NPC_Clickdb1 = {
	{625,80,4,53,nx,ny,0,"NhËn Tiªu Bang Héi",0,"\\script\\global\\quanlygame\\sukien\\vantieubanghoi\\vantieubanghoi.lua","main", 0},   
}
...
function call_npc(tbnpc) 
		for i = 1 , getn(tbnpc) do 
			...
			if (Mid >= 0 ) then 
				...
				AddTimer(25*60* 18, "OnTimeout", nNpcIdx);
					local szNews = format("<color=yellow>Nghe nãi <color=green>Tiªu Bang héi<color=yellow> ®· xuÊt hiÖn t¹i Ba L¨ng HuyÖn 184/205 bang héi nµo ®ñ b¶n lÜnh kÐo tiªu vÒ giao ë Ba L¨ng HuyÖn 218/213 sÏ ®­îc träng th­ëng.");
...
function call_npc1(nNpcIdx) 
  ...
  if (SId >= 0) then
		...
		local nHour = tonumber(GetLocalDate("%H%M"))
				local timemmm=2055-nHour
		AddTimer(timemmm*60* 18, "OnTimeout", nNpcIdx);
	  	if nNpcIndex > 0 then
...
local _GetFruit = function(nNpcIdx, dwNpcId)
 ...
 local szNewsnew = format("<color=cyan>Bang Chñ <color=green>"..GetName().."<color=yellow> ®o¹t thµnh c«ng b¾t ®Çu di chuyÓn tiªu Bang vÒ BiÖn Kinh 197/202 c¸c bang héi kh¸c h·y ra søc ®o¹t lÊy tiªu bang nµy!");
...
SetTask(5859,1)
ChangeOwnFeature( 1, 3500*18, 1903,  0, 0, 0, 0)
SetTaskTemp(200,1); 
ForbidEnmity(1);
SetPKFlag(1)
DisabledUseTownP(1);
...
ForbidChangePK(1);
SetMoveSpeed(6);
AddSkillState(464, 50, 0, 18*3000);
...
SetDeathScript("");
--KickOutSelf()
Msg2SubWorld("<color=cyan>Tiªu Bang cña Bang Chñ <color=green>"..GetName().."<color=yellow> BÞ h¹ thñ. Tiªu míi ®· xuÊt hiÖn l¹i ë ®iÓm xuÊt ph¸t. .!")	
end
