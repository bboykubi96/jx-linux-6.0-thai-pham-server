...
function main()
	...
	if (tbBirthday0905:IsActDate() == 1) then
		tinsert(tbDialog, 3, "? phong kş trªn b¶n ®å ®İch nhiÖm vô/birthday0905_fly");
	end
...
function fly_starttask()
	...
	elseif ( nt_getTask(1201) == 25 or nt_getTask(1201) == 30 ) then
		Describe(DescLink_YiGuan..": Ng­¬i ®· hoµn thµnh nhiÖm vô, thÕ nµo cßn muèn tiÕp tôc tiÕp nhËn vô ? nhanh lªn mét chót rêi ®i c¸i nµy ®i!",1,"KÕt thóc ®èi tho¹i/no")
