...
function add_shashouling(nvalue, series)
	...
	elseif ( nvalue >= 141 ) and ( nvalue<= 160  ) then
		...
		local diemdv = random(3,5)
 		AddRepute(diemdv)
		Msg2Player("B¹n nhËn ®­îc "..diemdv.." ®iÓm danh väng")
		...
		--tbAwardTemplet:GiveAwardByList({{szName = "S¸t thñ bÝ b¶o", tbProp = {6,1,2347,1,1,0}, nRate = 100}}, format("Get %s", "S¸t thñ bÝ b¶o"), 1)
		--tbAwardTemplet:GiveAwardByList({{szName = "Hµnh HiÖp lÖnh", tbProp = {6,1,2566,1,1,0}, nRate = 100, nCount = 5}}, format("Get %s", "S¸t thñ bÝ b¶o"), 1)

		--Msg2Player( "<color=cyan>B¹n nhËn ®­îc<color> <color=violet>5 TiÒn §ång (Khãa)<color> <color=cyan> tõ sù kiÖn Boss S¸t Thñ.<color>" );
		--jiefangri_award()
		end
end
